...
OQE ships an [MCP](https://modelcontextprotocol.io) server so you can talk
to your local OQE installation **directly from Claude Desktop or claude.ai
web** — no terminal needed. Claude sees the seven tools (the same set
`oqe llm-ask` uses), calls them when relevant, and answers in chat
...
```bash
poetry run oqe mcp-serve --help
```

```text
usage: oqe mcp-serve [-h] [--raw-only]
...
Replace `cwd` with the path where you cloned OQE. Restart Claude Desktop
after saving — the OQE tools will appear in the **Available Tools**
panel.

## Wire it into claude.ai web (custom integrations)

claude.ai now supports remote MCP integrations via the **Settings →
Connectors** page. Until your OQE install is reachable from the public
...
2. **Tunnel it** — expose your local server with `ngrok http 8000` (or
   ...
   HTTP transport too; if you'd like the OQE server to listen on HTTP
   instead of stdio, file an issue and we'll wire `oqe mcp-serve --http
   PORT`.

## What Claude sees

Once connected, Claude has access to the same seven tools as `oqe llm-ask`:
...
Drop the analytics tools (`oqe mcp-serve --raw-only`) if you want Claude
...
- Each tool call uses your Polygon API quota the same way `oqe ask` does.
  The `~/.oqe/cache.sqlite` cache is shared, so repeated questions about
  the same chain within the TTL are free.
- No OQE data leaves your machine except via the tool results you return
  to Claude.
...
| --- | --- | --- |
| Tools don't appear in Claude Desktop | wrong `cwd` or `command` | Check the absolute path; run `poetry run oqe mcp-serve` from a terminal first to confirm it boots. |
...
| Server starts but tools error out | stale cache | `rm ~/.oqe/cache.sqlite` and reload Claude Desktop. |
...
- [Architecture: caching](../architecture/caching.md) — how the cache the MCP server shares with `oqe ask` is keyed.
