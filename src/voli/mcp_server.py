...
Exposes the Voli tools to any MCP client - Claude Desktop, claude.ai web
custom integrations, Continue, Zed, etc. The same `build_default_tools()`
that powers `voli llm-ask` is reused here, so the analytics + raw layers
the LLM has access to via our CLI are exactly the same set Claude sees
when it talks to Voli through MCP.
...
Restart Claude Desktop and the Voli tools appear in the Available Tools
panel.
"""
...
SERVER_NAME = "voli"
SERVER_DESCRIPTION = (
    "Voli - grounded options chain tools (snapshot, contracts, "
...
def _to_mcp_tool(tool: ToolDef, types_module) -> Any:
    """Convert one Voli ToolDef into the MCP types.Tool shape."""
...
def _build_server(*, include_analytics: bool = True):
    ...
    async def _call_tool(name: str, arguments: dict[str, Any] | None) -> list:
        # Voli tools are sync functions that hit Polygon (which is also
