"""Command-line interface for the Voli.

Usage:
    voli ask "NVDA ATM IV this week vs next week" [--ticker NVDA]
...
The CLI is a thin shell around `voli.agent.answer_question`. The orchestration
...
# Library users who import voli.* directly are unaffected - dotenv only fires
# when this CLI module is imported.
load_dotenv()
# Then layer config.yaml on top (env vars already set still win).
load_config()
# Configure the `voli` logger so any module that imports `from voli.logging
...
def _add_theme_flags(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        metavar="NAME",
        help="Colour theme. Default: bloomberg (or $VOLI_THEME).",
    )
    parser.add_argument(
        "--cycle-theme",
        action="store_true",
        help="Pick the next theme in rotation (state in ~/.voli/theme_cursor).",
    )
    parser.add_argument(
        "--no-color",
        action="store_true",
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="voli",
        description=(
            "Voli - ask grounded questions about an options "
    ...
    ask.add_argument(
        "--trace",
        action="store_true",
        help="Open a JSONL run-trace under $VOLI_TRACE_DIR for this question.",
    )
    ask.add_argument(
        "--skeptic",
        action="store_true",
    ...
    llm = sub.add_parser(
        "llm-ask",
        help="Ask an LLM (Claude or GPT) using Voli tools as its data backend.",
    )
    ...
    llm.add_argument(
        ...
        help="LLM provider. Default: $VOLI_LLM_PROVIDER, or anthropic/openai "
        "based on which API key is set.",
    )
    llm.add_argument(
        "--model",
        default=None,
        help="Model name (overrides $VOLI_LLM_MODEL and the provider default).",
    )
    llm.add_argument(
    ...
    mcp_serve = sub.add_parser(
        "mcp-serve",
        help="Run the Voli Model Context Protocol server over stdio.",
    )
...
def _selected_theme(args: argparse.Namespace) -> str:
    """Apply --theme / --cycle-theme / VOLI_THEME precedence.

    Order of preference: --cycle-theme > --theme > $VOLI_THEME > 'bloomberg'.
    """
...
def _extract_cache_marker(content: str) -> str:
    """Pull a 'served-from' label out of a tool's JSON result string.

    Raw Polygon tools nest the field as ``meta.primary_source``; analytics
    tools (one bulk fetch per call) emit it as a flat ``primary_source``
    field. Returns ``"cache"``, ``"polygon"``, ``"mixed"``, or ``""`` if the
    payload isn't JSON or doesn't carry the field. Used purely for
    presentation - the field already shipped to the LLM verbatim.
    """
    ...
    if not isinstance(payload, dict):
        return ""
    if isinstance(payload.get("primary_source"), str):
        return payload["primary_source"]
    meta = payload.get("meta")
    if isinstance(meta, dict) and isinstance(meta.get("primary_source"), str):
        return meta["primary_source"]
    return ""
        ...
        for event in llm_ask(args.prompt, provider=provider, tools=tools, config=cfg):
            if isinstance(event, ToolCallStart):
                ...
                if event.is_error:
                    ...
                    src = _extract_cache_marker(event.content)
                    marker = f"[ TOOL OK   ({src}) ]" if src else "[ TOOL OK   ]"
                print(
...
def _cmd_mcp_serve(args: argparse.Namespace, *, out) -> int:
    """Run the Voli MCP server over stdio.
...
def _cmd_replay(args: argparse.Namespace, *, out) -> int:
    ...
    Auto-detects two companion shapes:
      * <id>.response.json  - rule-based AnswerResponse (voli.replay.dump_response)
      * <id>.llm.json       - LLM run record (voli.llm.replay.dump_llm_run)
    """
...
def _replay_llm_run(args: argparse.Namespace, record, *, out) -> int:
    ...
    for call, result in zip(record.tool_calls, record.tool_results, strict=False):
        ...
        if result.get("is_error"):
            ...
            src = _extract_cache_marker(result["content"])
            marker = f"[ TOOL OK   ({src}) ]" if src else "[ TOOL OK   ]"
...
def _sample_response() -> AnswerResponse:
    ...
    output so previews look identical to a real `voli ask` run.
    """
...
def _render_error(args: argparse.Namespace, exc: Exception, *, out) -> int:
    ...
    print(theme.status_bar(f"VOLI | ERROR: {cls}"), file=out)
    print(theme.dim("=" * 80), file=out)
...
if __name__ == "__main__":  # pragma: no cover - exercised via `voli` script
    sys.exit(main())
