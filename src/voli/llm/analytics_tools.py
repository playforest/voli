...
These wrappers hide that plumbing. Each one:
  ...
  2. Calls the corresponding pure function in `voli.analytics`.
...
def _combine_sources(*sources: str) -> str:
    """Reduce per-fetch sources into a single label for the LLM.

    All-"cache" stays "cache"; all-"polygon" stays "polygon"; anything
    else (one of each) is reported as "mixed" so the operator can see
    that part of the chain came from the network even if the rest hit
    the cache.
    """
...
def _fetch_chain(
    *,
...
) -> tuple[float, list, dict, dict, str]:
    """Return (spot, contracts, greeks_by_symbol, quotes_by_symbol, source).

    `right_word` must be normalised ("call" / "put"). `expiry` is an
    ISO YYYY-MM-DD string or None. `source` is "cache", "polygon", or
    "mixed" depending on whether the underlying snapshot + chain pull
    were both served from cache.
    ...
    snap_source = getattr(getattr(snap, "meta", None), "primary_source", "polygon")

    rf = _right_filter(right_word)
    contracts, quotes_by_symbol, greeks_by_symbol, chain_source = get_option_chain_bulk(
        ticker=ticker,
        right=rf,
        expiry=expiry,
    )

    source = _combine_sources(snap_source, chain_source)
...
def _tool_compute_atm_iv_term_structure(args: dict[str, Any]) -> str:
    ...
    spot, contracts, greeks, quotes, source = _fetch_chain(
        ticker=ticker,
        right_word=right_word,
        include_quotes=max_spread is not None,
    )
    if not contracts:
        return _err("no_contracts", ticker=ticker, primary_source=source)
...
def _tool_get_atm_greeks(args: dict[str, Any]) -> str:
    ...
    spot, contracts, greeks, _, source = _fetch_chain(
        ticker=ticker,
        right_word=right_word,
        expiry=expiry,
