...
Existing modules read settings from VOLI_* env vars (cache path, trace dir,
...
_KEY_TO_ENV: dict[str, str] = {
    "cache_path": "VOLI_CACHE_PATH",
    "trace_dir": "VOLI_TRACE_DIR",
    "default_theme": "VOLI_THEME",
    "theme_cursor_path": "VOLI_THEME_CURSOR",
    "log_level": "VOLI_LOG_LEVEL",
    "log_format": "VOLI_LOG_FORMAT",
...
def _candidate_paths() -> list[Path]:
    out: list[Path] = []
    env = os.environ.get("VOLI_CONFIG")
    if env:
        out.append(Path(env).expanduser())
    out.append(Path("config.yaml"))
    out.append(Path("~/.voli/config.yaml").expanduser())
    return out
...
def load_config(*, override_env: bool = False) -> VoliConfig:
    ...
    for candidate in _candidate_paths():
        ...
        for key, val in raw.items():
            ...
            applied[key] = str(val)
        return VoliConfig(source_path=candidate, values=applied)

    return VoliConfig(source_path=None, values={})
