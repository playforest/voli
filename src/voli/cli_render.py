...
of this module) - users select with `voli ask --theme NAME` or `VOLI_THEME=NAME`.
"""
...
def resolve_theme_name(name: str | None) -> str:
    """Validate `name` (or fall back to VOLI_THEME or the default)."""

    if name is None:
        name = os.environ.get("VOLI_THEME") or DEFAULT_THEME
...
def make_theme(color: bool | None = None, *, theme: str | None = None) -> Theme:
    """Build a Theme. `color=None` auto-detects, `theme=None` uses VOLI_THEME
...
def render_batch(
    batch: Any,  # voli.agent.batch.BatchResult (avoid import cycle)
    *,
    color: bool | None = None,
    theme: str | None = None,
    asof: str | None = None,
...
# ---------------------------------------------------------------------------
# Theme cycling: lightweight per-user state at ~/.voli/theme_cursor so
# successive `voli ask --cycle-theme` invocations rotate through THEMES in
# definition order.
# ---------------------------------------------------------------------------


def _theme_cursor_path() -> str:
    override = os.environ.get("VOLI_THEME_CURSOR")
    if override:
        return os.path.expanduser(override)
    return os.path.expanduser("~/.voli/theme_cursor")
