...
Two formats, picked by `VOLI_LOG_FORMAT` (or config.yaml `log_format`):
...
Level via `VOLI_LOG_LEVEL` (default `WARNING`). The CLI calls `setup_logging()`
once at startup; library users who import voli.* get no implicit logging
...
def _level_from_env() -> int:
    name = os.environ.get("VOLI_LOG_LEVEL", _DEFAULT_LEVEL).upper()
    return _LEVELS.get(name, logging.WARNING)


def _format_from_env() -> str:
    return os.environ.get("VOLI_LOG_FORMAT", _DEFAULT_FORMAT).lower()
...
def setup_logging(
    *,
    ...
    """Configure the `voli` logger and return it.
    ...
    if theme_name is None:
        theme_name = os.environ.get("VOLI_THEME") or "bloomberg"
...
def get_logger(name: str = "voli") -> logging.Logger:
    """Return a child logger of the `voli` namespace.

    Example:
        from voli.logging import get_logger
        log = get_logger("voli.eval")
        log.info("starting eval ...")
    """

    if not name.startswith("voli"):
        name = f"voli.{name}"
