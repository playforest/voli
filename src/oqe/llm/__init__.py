"""LLM-driven agent: an LLM (Claude or GPT) plans + answers using OQE tools.

This is an alternative to the rule-based `oqe.agent` pipeline. Both share
the same tool layer (`oqe.tools.polygon_tools`) so they can never disagree
