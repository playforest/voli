...
When `oqe llm-ask --trace` is set we drop a JSON file alongside the
existing JSONL flight recorder so `oqe replay` can re-render the LLM
...
from oqe.run_trace import default_trace_dir
