...
prompt through `oqe.agent.answer_question` against the synthetic registry, and
...
def render_report(
    ...
    lines.append(
        t.status_bar(
            f"OQE EVAL | {report.total} cases | {report.passed} passed | {report.failed} failed"
        )
    )
    lines.append(t.dim("=" * width))
...
def main(
    ...
    *,
    ...
    parser = argparse.ArgumentParser(prog="oqe-eval", description="Run the OQE evaluation harness.")
    parser.add_argument(
